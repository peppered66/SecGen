class redisdb_rce::install {

 $secgen_parameters = secgen_functions::get_parameters($::base64_inputs_file)
 $strings_to_leak = $secgen_parameters['strings_to_leak']
 $leaked_filenames = $secgen_parameters['leaked_filenames']
 $strings_to_store = $secgen_parameters['strings_to_store']

 $redistext1 = $strings_to_store[0]
 $redistext2 = $strings_to_store[1]  
 ...
 #Replaces original config file for misconfiguration
  file { '/etc/redis/redis.conf':
    ...
    source  => 'puppet:///modules/redisdb_rce/redis.conf',
    owner   => 'redis',
    group   => 'redis',
    mode    => '0640',
 ...
 #Stores file1 on disk
  ...
  content  => template('redisdb_rce/redistext1.txt.erb'),
  owner   => 'root',
  group   => 'root',
  mode    => '0644',
 ...
 #Stores file2 on disk
  ...
  content  => template('redisdb_rce/redistext2.txt.erb'),
  owner   => 'root',
  group   => 'root',
  mode    => '0644',
  require => File['/opt/redis-files'],
...
exec { 'remove-corrupt-rdb':
 ...
 ::secgen_functions::leak_files { 'redis-db-flag-leak':
    ...
    owner             => 'redis',
    mode              => '0600',
    leaked_from       => 'redis-db-flag-leak',
  }
 }
